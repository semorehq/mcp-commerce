...
import { z } from "zod";
export const SUPPORTED_LANGS = ["en", "ja", "de", "fr", "es", "ko"];
...
const EMPTY_SEARCHER = async () => ({ results: [], total: 0 });
export function makeSearchProductTool(options = {}) {
    const searcher = options.searcher ?? EMPTY_SEARCHER;
    return {
        name: "search_product",
        description: "Search the Semore K-product catalog by keyword, category, price range, and destination. Returns ranked SKU offers (id, title, price USD, ship-from, availability).",
        inputSchema: SearchProductInput,
        outputSchema: SearchProductOutput,
        handler: async (input) => searcher(input),
    };
}
