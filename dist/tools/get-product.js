...
const STUB_FETCHER = async (input) => ({
    sku_id: input.sku_id,
...
export function makeGetProductTool(options = {}) {
    const fetcher = options.fetcher ?? STUB_FETCHER;
    return {
        name: "get_product",
        description: "Fetch a single SKU's i18n detail: title, description, images, specs, weight, HS code, ship-from, lead time, and live stock.",
        inputSchema: GetProductInput,
        outputSchema: GetProductOutput,
        handler: async (input) => fetcher(input),
    };
}
